...
find_column() {
    ...
    local index=1
    IFS="$delimiter" read -ra fields <<< "$header_line"
    
    for field in "${fields[@]}"; do
        field_lower=$(echo "$field" | tr '[:upper:]' '[:lower:]')
        if echo "$field_lower" | grep -q "$pattern"; then
            echo "$index"
            return 0
        fi
        ((index++))
...
parse_csv() {
    ...
    # Find column indexes for key fields with simpler patterns
    STORE_COL=$(find_column "$header_line" "$delimiter" "loja")
    POS_COL=$(find_column "$header_line" "$delimiter" "pos")
    PAY_COL=$(find_column "$header_line" "$delimiter" "pag")
    DATE_COL=$(find_column "$header_line" "$delimiter" "dia")
    TRANS_COL=$(find_column "$header_line" "$delimiter" "trans")
